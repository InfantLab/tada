...
test.describe('Meditation Timer Flow', () => {
  test('should complete meditation and create entry', async ({ page }) => {
    ...
    await expect(page.locator('[data-test="timer-display"]')).toContainText('0:02')
    
    // Wait for completion
    await page.waitForSelector('[data-test="completion-bell"]', { timeout: 3000 })
    
    // Verify entry was created
    await page.goto('/timeline')
    await expect(page.locator('[data-test="entry-type"]').first()).toContainText('meditation')
  })
  ...
  test('should pause and resume timer', async ({ page }) => {
    ...
    await page.waitForTimeout(1000)
    const timeAfterResume = await page.textContent('[data-test="timer-display"]')
