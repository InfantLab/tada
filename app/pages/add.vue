...
// Entry types
const entryTypes = [
  { value: 'tada', label: 'Tada!', emoji: '🎉', description: 'Celebrate an accomplishment' },
  { value: 'dream', label: 'Dream', emoji: '🌙', description: 'Record a dream' },
  { value: 'note', label: 'Note', emoji: '📝', description: 'Capture a thought' },
  { value: 'meditation', label: 'Meditation', emoji: '🧘', description: 'Log a session' },
]
...
const emotionOptions = ['joy', 'fear', 'wonder', 'anxiety', 'peace', 'confusion', 'love', 'anger', 'sadness']
...
<template>
  <div class="max-w-lg mx-auto">
    ...
    <div class="flex items-center gap-4 mb-6">
      ...
      <h1 class="text-2xl font-bold text-stone-800 dark:text-stone-100">New Entry</h1>
    </div>
    ...
    <form @submit.prevent="submitEntry" class="space-y-6">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <button
            ...
            :class="entryType === type.value 
              ? 'border-tada-500 bg-tada-50 dark:bg-tada-900/20' 
              : 'border-stone-200 dark:border-stone-700 hover:border-stone-300 dark:hover:border-stone-600'"
          >
            <div class="flex items-center gap-2 mb-1">
              <span class="text-xl">{{ type.emoji }}</span>
              <span class="font-medium text-stone-800 dark:text-stone-100">{{ type.label }}</span>
            </div>
            <p class="text-xs text-stone-500 dark:text-stone-400">{{ type.description }}</p>
          </button>
        </div>
      </div>
      ...
      <!-- Title -->
      <div>
        <label for="title" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-2">
          Title
        </label>
        <input
          id="title"
          v-model="title"
          type="text"
          :placeholder="entryType === 'tada' ? 'What did you accomplish?' : entryType === 'dream' ? 'What was the dream about?' : 'Give it a title...'"
      ...
      <!-- Notes -->
      <div>
        <label for="notes" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-2">
          Notes
        </label>
        <textarea
          id="notes"
          v-model="notes"
          rows="4"
          :placeholder="entryType === 'dream' ? 'Describe the dream in detail...' : 'Add more details...'"
      ...
      <template v-if="entryType === 'dream'">
        ...
        <div class="flex items-center justify-between">
          <label for="lucid" class="text-sm font-medium text-stone-700 dark:text-stone-300">
            Lucid dream?
          </label>
          <button
            type="button"
            ...
            :class="dreamData.lucid ? 'bg-tada-500' : 'bg-stone-300 dark:bg-stone-600'"
            role="switch"
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              :key="emotion"
              type="button"
              @click="dreamData.emotions.includes(emotion) 
                ? dreamData.emotions = dreamData.emotions.filter(e => e !== emotion)
                : dreamData.emotions.push(emotion)"
              class="px-3 py-1.5 rounded-full text-sm font-medium transition-colors"
              :class="dreamData.emotions.includes(emotion)
