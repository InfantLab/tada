...
/**
 * MomentInput — Core input component for the moment entry type module.
 *
 * Features:
 * - Subcategory types: magic, dream, gratitude, journal
 * - Voice input with LLM extraction
 * - Dream-specific fields (vividness, lucid)
 * - Ta-da extraction from voice input
 * - Recent moments list with type filtering
 */
...
async function handleVoiceComplete(
  ...
  try {
    ...
    // Only keep ta-das that look like actual accomplishments
    if (extraction.tadas && extraction.tadas.length > 0) {
