...
/**
 * TadaInput — Core input component for the ta-da entry type module.
 *
 * Features:
 * - Single and multi ta-da entry
 * - Voice input with LLM extraction
...
function populateMultiTadaMode(tadas: ExtractedTada[]) {
  multiTadaList.value = tadas.map((t) => ({
    ...
    notes: t.notes || "",
    emoji: "⚡",
    category: t.category || tadaCategory.value,
...
function guessSubcategory(title: string): string | null {
  ...
  if (
    /\b(work|job|office|meeting|client|project|deadline|report|email|boss|colleague|presentation|code|deploy|release|sprint|ticket|pr|review)\b/.test(lower)
  ) return "work";

  if (
    /\b(home|house|clean|cook|laundry|dishes|garden|yard|repair|fix|organize|declutter|groceries|kitchen|bedroom|bathroom)\b/.test(lower)
  ) return "home";

  if (
    /\b(exercise|workout|gym|run|walk|yoga|meditat|health|doctor|appointment|medicine|sleep|water|diet|weight|stretch|physio|therapy)\b/.test(lower)
  ) return "health";

  if (
    /\b(friend|family|call|visit|party|dinner|lunch|coffee|catch up|meet|hangout|birthday|anniversary|gift)\b/.test(lower)
  ) return "social";

  if (
    /\b(read|book|paint|draw|music|play|game|craft|hobby|learn|course|class|practice|piano|guitar|photo|write|blog)\b/.test(lower)
  ) return "hobby";
