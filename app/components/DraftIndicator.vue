...
const {
  ...
  refresh,
  status,
} = useFetch<{ drafts: Draft[]; total: number }>("/api/entries/drafts", {
...
function getDraftName(input: Record<string, unknown>): string {
  return (
    ...
    (input["parsedFrom"] as string)?.slice(0, 30) ||
    "Untitled draft"
  );
}
...
<template>
  <div class="draft-indicator relative">
    ...
    >
      ...
      <span v-else-if="status === 'pending'" class="text-xs"> Loading... </span>
    </button>
    ...
    >
      ...
      >
        <!-- Header -->
        <div
          class="px-3 py-2 border-b border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-900/50"
        >
        ...
        <div class="max-h-64 overflow-y-auto">
          ...
          >
            <div class="flex items-center justify-between">
              <span
                class="text-sm font-medium text-stone-900 dark:text-white truncate"
              >
            ...
            <div class="flex items-center gap-2 mt-0.5">
              ...
              >
                "{{ draft.parsedFrom.slice(0, 30)
                }}{{ draft.parsedFrom.length > 30 ? "..." : "" }}"
              </span>
            </div>
          </button>
        </div>
        ...
        <!-- Footer -->
        <div
          class="px-3 py-2 border-t border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-900/50"
        >
