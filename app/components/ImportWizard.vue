...
async function saveRecipe(config: {
  ...
  await $fetch<unknown>("/api/import/recipes", {
    method: "POST",
    body: config,
  });
...
async function restoreVersion(versionIndex: number) {
  ...
  try {
    ...
    if (response.success && response.recipe) {
      ...
      columnMapping.value = response.recipe["columnMapping"] as Record<
        string,
        string
      >;
      transforms.value = {
      ...
      currentRecipePreviousVersions.value =
        (response.recipe["previousVersions"] as Array<{
          savedAt: string;
          columnMapping: Record<string, unknown>;
          transforms: Record<string, unknown>;
        }>) || [];
...
async function deleteSelectedRecipe() {
  ...
  try {
    await $fetch<unknown>(`/api/import/recipes/${selectedRecipeId.value}`, {
      method: "DELETE",
    });
